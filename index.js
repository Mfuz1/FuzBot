...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `ara-ara SAYONARA @${num.split('@')[0]} kalau balik nitip seblak pake ceker yaa`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
               ...
               case 'help': 
				...
				case 'limit':
				   ...
				   checkLimit(sender)
					break
               case 'giftlimit': 
				if (!isOwner) return reply(ind.premon(pushname))
...
• Limit: ${limitawal-updated.limit}`
                ...
                await reply('Sabarrr,nyari pemenang...')
                await reply(`wa.me/${anug}`)
                await reply( `Pemenang ditemukan: 🙉\n*${prefix}next* — cari pemenang lagi`)
            break
            case 'next':
                if (!isRegistered) return reply( ind.noregis())
                if (!isOwner) return reply(ind.ownerb()
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'simi':
				case 'empus':
					if (args.length < 1) return reply('hadirrrrr')
					teks = body.slice(5)
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan oleh owner MFUZ`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena anda owner FUZ-BOT mengirim ${one}Xp untuk anda`)
                      }else{
